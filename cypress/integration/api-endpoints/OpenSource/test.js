...
describe("Test Endpoints and basic Security", () => {
    it("Check if endpoints to test", () => {
        endpoints = controller.orderedEndpointData.filter((x) => {
            return x.tags.includes("OpenSource");
        });
...
describe("Test Add View Method", () => {
    it("Check method success 200", () => {
        const newendpoint = "/opensource/addView/";
        const type = {
            requestType: "post",
            endpoint: newendpoint,
        };
        cy.apiRequest(type, null, { page: "testpage", parent: settings.apiURL }).then((response) => {
...
describe("Test Get By Page Method", () => {
    it("Check method success 200", () => {
        const newendpoint = "/opensource/getByPage/";
        const type = {
            requestType: "post",
            endpoint: newendpoint,
        };
        cy.apiRequest(type, JWTs.userJWT, { page: "testpage", limit: "10" }).then((response) => {
            cy.expect(response.status).to.be.equal(200);
        });
    });
