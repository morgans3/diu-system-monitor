...
describe("Test Endpoints and basic Security", () => {
    it("Check if endpoints to test", () => {
        endpoints = controller.orderedEndpointData.filter((x) => {
            return x.tags.includes("TeamRequests");
        });
...
describe("Test GET Methods", () => {
    it("Check get all", () => {
        const newendpoint = "/teamrequests/";
        const type = {
            requestType: "get",
    ...
    it("Check get by params - ID", () => {
        const newendpoint = "/teamrequests/" + request["id"];
        const type = {
            requestType: "get",
            endpoint: newendpoint,
        };
    ...
    it("Check get by params - Username", () => {
        const newendpoint = "/teamrequests/username/" + JWTs.username;
        const type = {
            requestType: "get",
            endpoint: newendpoint,
        };
    ...
    it("Check get by params - Teamcode", () => {
        const newendpoint = "/teamrequests/teamcode/" + shellTeam.code;
        const type = {
            requestType: "get",
            endpoint: newendpoint,
        };
...
describe("Test PUT Method", () => {
    ...
    it("Test for Unsuccessful update (403)", () => {
        request.organisation = "Test";
        cy.apiRequest(putEndpoint, JWTs.adminJWT, request).then((response) => {
    ...
    it("Test for Successful update (200)", () => {
        request.organisation = "Test";
        cy.apiRequest(putEndpoint, JWTs.userJWT, request).then((response) => {
            cy.expect(response.status).to.be.equal(200);
        });
    });
});
...
describe("Test DELETE Method", () => {
    ...
    it("Test for Unsuccessful update (403)", () => {
        cy.apiRequest(deleteEndpoint, JWTs.adminJWT, request).then((response) => {
            cy.expect(response.status).to.be.equal(403);
        });
    });
    ...
    it("Test for Successful delete (200)", () => {
        cy.wait(500); // write/read delayed for DynamoDB
        cy.apiRequest(deleteEndpoint, JWTs.userJWT, { id: request["id"], teamcode: shellTeam.code }).then((response) => {
            cy.expect(response.status).to.be.equal(200);
        });
    });
